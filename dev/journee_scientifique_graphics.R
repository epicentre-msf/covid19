...
ggplot2::theme_set(theme_light(base_size = 16, base_family = font))
...
# data =========================================================================================
df_covid <- df_ecdc %>% filter(country %in% c("Burkina Faso", "Niger", "Mauritania", "Chad", "Mali"))
...
p_map_cases <- ggplot(df_map) +
  ...
  geom_sf_label(aes(label = country), nudge_y = 2, fontface = "bold", fontfamily = font) +
  geom_text(aes(lon, lat, label = frmt_num(cases, .1)), colour = "white", fontface = "bold", fontfamily = font) +
  coord_sf(datum = NA) +
...
p_map_deaths <- ggplot(df_map) +
  ...
  geom_sf_label(aes(label = country), nudge_y = 2, fontface = "bold", fontfamily = font) +
  geom_text(aes(lon, lat, label = frmt_num(deaths)), colour = "white", fontface = "bold", fontfamily = font) +
  coord_sf(datum = NA) +
  theme_minimal(base_size = 16, base_family = font) +
  labs(x = NULL, y = NULL, size = "Décès", caption = caption, title = "") +
  theme(legend.position = "none", plot.caption = element_text(face = "italic", size = 10), ) +
  #theme(legend.position = c(0.55, .9)) +
  guides(
    size = guide_legend(nrow = 1, label.position = "bottom", title.position = "left")
...
p_weekly_cases <- ggplot(df_weekly, aes(week_date, cases, group = country)) +
  ...
  scale_x_date("Semaine", date_breaks = "2 weeks", date_labels = "%V", sec.axis = sec_axis(trans = ~ .), expand = expansion(mult = c(0.02, 0.02))) +
